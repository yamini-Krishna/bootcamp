...
def main():
    ...
    for pmcid in pmcids:
        ...
        try:
            ...
            logger.error(f"Failed to fetch XML for {pmcid}: {e}")
            continue

        if not xml_data:
            logger.error(f"No data retrieved for PMCID: {pmcid}. Skipping.")
            continue
    ...
    if args.format == 'csv':
        exporter = CSVExporter()
        exporter.save_to_csv(all_parsed_data, args.file)
    elif args.format == 'json':
        exporter = JSONExporter()
        exporter.save_to_json(all_parsed_data, args.file)
